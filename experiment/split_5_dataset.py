    ...
    for i in samples_list:
        if os.path.exists(data_files + i + '.json'):
            ...
            if data['disease_united'] in disease_list:
                if data['disease_united'] == phe_df.loc[i, 'disease_united'] and data['group'] == phe_df.loc[
                    i, 'group']:
                    files_list.append(data_files + i + '.json')
                else:
                    print('Label Conflict: ', i, data['disease_united'], phe_df.loc[i, 'disease_united'], data['group'],
                          phe_df.loc[i, 'group'])
            else:
...
if __name__ == '__main__':
    ...
    for seed in [111, 222, 333, 444, 555]:
        ...
        phe_ctrl_df = phe_df[
            (phe_df['project'].isin(['LifeLinesDeep_2016', 'MetaCardis_2020_a'])) & (
                        phe_df['disease_united'] == 'healthy')]
        ...
        pandisease_rest_train, pandisease_rest_val_test = stratifed_split_pandisease_rest(seed, phe_filter_CRC_IBD_df,
                                                                                          0.2)

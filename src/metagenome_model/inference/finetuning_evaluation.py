...
class MetaGenomeForPhenotypeInfer(Kernel):
    ...
    def inference(self, **kwargs):
        ...
        if self.accelerator.is_main_process:
            ...
            # result.round(4).to_csv(self.output_home + '/pandisease.all.result.csv')
            # #
            preds = [0 if state_preds[i] == 0 else preds[i].item() + 1 for i in range(len(state_preds))]
            MCC = matthews_corrcoef(labels.numpy(), preds)
            report = classification_report(y_true=labels.numpy(), y_pred=preds, output_dict=True)
            ...
            # multi_result.round(4).to_csv(self.output_home + '/multidisease.all.result.csv')
            # #
            # # save probability
            prob_df = pd.DataFrame(
                {'sample': name, 'prob': state_proba, 'pred': state_preds.numpy(), 'label': state_labels.numpy()})
            # prob_df.to_csv(self.probs_path + '/pandisease.all.prob.csv', index=False)
            # #
            prob_df = pd.DataFrame(proba)
            prob_df.columns = list(self.PAN_LABELS.keys())[1:]
            prob_df.index = name
            prob_df['label'] = labels.numpy()
            prob_df['pred'] = preds
            # prob_df.to_csv(self.probs_path + '/multidisease.all.prob.csv')
